...
export default function CommentSection({ postId, initialComments }: CommentSectionProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createComment(postId, content, authorName, authorEmail);
            setContent('');
            setAuthorName('');
            setAuthorEmail('');
            setMessage({
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="flex items-center space-x-4 mb-4">
                        ...
                        <span className="text-sm font-black text-slate-900 uppercase tracking-wider">
                            {session ? (session.user?.name || 'Contributor') : (authorName || 'Guest')}
                        </span>
                    </div>
                    ...
                    {!session && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <input
                                ...
                                className="w-full bg-white border border-slate-100 rounded-2xl px-6 py-4 text-slate-700 outline-none focus:ring-4 focus:ring-blue-100/50 focus:border-blue-200 transition-all text-sm shadow-sm"
                                required
                            />
                            <input
                                type="email"
                        ...
                        <div key={comment.id} className="flex space-x-6 group">
                            <div className="flex-shrink-0">
                                <div className="w-12 h-12 rounded-3xl bg-white border border-slate-100 flex items-center justify-center text-xs font-black text-slate-400 uppercase shadow-sm group-hover:text-blue-600 group-hover:border-blue-100 transition-all">
                                    {(comment.user?.name || comment.user?.email || comment.authorName || 'A').charAt(0)}
                                </div>
                            </div>
                            <div className="flex-1 bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm group-hover:shadow-md transition-all duration-300">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">
                                        {comment.user?.name || comment.authorName || 'Anonymous Intelligence'}
                                    </h4>
